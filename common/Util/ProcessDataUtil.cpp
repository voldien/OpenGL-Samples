...
void ProcessData::computeBump2Normal(unsigned int bump_source, unsigned int &normal_target, const unsigned int width,
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_BASE_LEVEL, 0);
	glBindTexture(GL_TEXTURE_2D, 0);
	ProcessData::computeIrradiance(bump_source, normal_target);
}
...
void ProcessData::computeBump2Normal(unsigned int bump_source, unsigned int normal_target) {
	...
	/*	Parameters.	*/
	glUniform1f(glGetUniformLocation(this->bump2normal_program, "settings.SampleDelta"), 0.025f);
	glUniform1f(glGetUniformLocation(this->bump2normal_program, "settings.maxValue"), 64);

	glUniform1i(glGetUniformLocation(this->bump2normal_program, "sourceEnvTexture"), 0);
	glUniform1i(glGetUniformLocation(this->bump2normal_program, "targetIrradianceTexture"), 1);
	...
	/*	The image where the graphic version will be stored as.	*/
	glBindImageTexture(1, normal_target, 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_RGBA32F);
