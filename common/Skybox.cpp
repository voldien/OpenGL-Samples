...
namespace glsample {
	...
	void Skybox::Init(unsigned int texture, unsigned int program) {
		...
		glSamplerParameteri(this->skybox_sampler, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
		glSamplerParameterf(this->skybox_sampler, GL_TEXTURE_LOD_BIAS, 0.0f);
		glSamplerParameterf(this->skybox_sampler, GL_TEXTURE_MAX_ANISOTROPY_EXT, 2.0f);
