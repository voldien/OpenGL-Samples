...
// vec4 blurHorizontal(const in float radius, const in int half_samples) {
...
// 	/*	Vertical Blur.	*/
// 	const vec4 horizontalBlur = blurHorizontal(settings.radius, settings.samples / 2);
// 	imageStore(TargetTexture, TexCoord, vec4(horizontalBlur.rgb, 1.0));
// }