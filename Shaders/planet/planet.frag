...
void main() {
	...
	const vec2 uvTex = sphere_uv_mapping(in_WorldPosition.xyz);

	/*	Convert normal map texture to a vector.	*/
	vec3 NormalMapBump = 2.0 * texture(NormalTexture, uvTex).xyz - vec3(1.0, 1.0, 1.0);
	/*	Scale non forward axis.	*/
	NormalMapBump.xy *= 10; /*  TODO: fix.  */
	...
	const vec4 terrainColor = texture(DiffuseTexture, uvTex);
