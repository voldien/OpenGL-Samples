...
void main() {
	...
	const float EyeToVertexDistance0 = distance(ubo.camera.position.xyz, (ubo.view * vec4(oPatch.WorldPos_B030, 1)).xyz);
	const float EyeToVertexDistance1 = distance(ubo.camera.position.xyz, (ubo.view * vec4(oPatch.WorldPos_B021, 1)).xyz);
	const float EyeToVertexDistance2 = distance(ubo.camera.position.xyz, (ubo.view * vec4(oPatch.WorldPos_B201, 1)).xyz);
